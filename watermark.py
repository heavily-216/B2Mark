...
def generate_green_domains(
    min_val: float, max_val: float, k: int, seed: int
...
def insert(
    ...
    embed_count = {i: 0 for i in range(num_bits)}
...
def detect(
    ...
    *,
    ...
    if embed_metadata is None:
        raise ValueError(
            "검출에는 삽입 시 반환된 metadata(dict: min, max, seed)가 필요합니다."
        )
    ...
    for i in range(bit_length):
        ...
        if t_cnt == 0:
            detected_id += "?"
            if verbose:
                print(f"  비트 {i}: 검사할 데이터 없음 (?) ")
            continue
        z_score = calculate_z_score(g_cnt, t_cnt)
        z_scores.append(z_score)
        # 1.645를 넘기면 해당 자리의 검출된 비트는 1
        bit_result = "1" if z_score > 1.645 else "0"
        detected_id += bit_result

        if verbose:
            print(
                f"  비트 {i}: green={g_cnt}, total={t_cnt}, z_score={z_score:.3f} → {bit_result}"
            )
...
class B2MarkEmbedder:
    ...
    def embed(
        ...
        result = insert(source_path, output_path, options, verbose=verbose)
...
class B2MarkDetector:
    ...
    def detect(
        ...
        result = detect(
            suspect_path, options, embed_metadata=meta_data, verbose=verbose
        )
        return result.detected_bitstring
    ...
    def verify_ownership(
        ...
        result = detect(suspect_path, options, embed_metadata=meta_data)
        detected_id = result.detected_bitstring

        # 매칭된 비트 계산
        matched_bits = sum(
            1
            for i in range(bit_length)
            if i < len(detected_id)
            and i < len(claimed_buyer_id)
            and detected_id[i] == claimed_buyer_id[i]
        )
        known_bits = sum(
            1
            for i in range(bit_length)
            if i < len(detected_id) and detected_id[i] != "?"
        )
        match_ratio = matched_bits / known_bits if known_bits > 0 else 0
        ...
        return {
            ...
            "ownership_verified": match_ratio >= min_match_ratio,
